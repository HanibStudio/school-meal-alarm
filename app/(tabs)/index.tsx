...
function HomeScreen() {
  ...
  const { selectedSchool, selectedMealTypes, favoriteDishKeywords } = useSettings();
              ...
              {isLoading ? (
                ...
                <View style={styles.menuList}>
                  {menuItems.map((item, idx) => {
                    ...
                    const isFavorite = isDishFavorite(item, favoriteDishKeywords);
                    const matchingKeywords = getMatchingKeywords(item, favoriteDishKeywords);

                    return (
                      <View key={idx} style={[styles.menuItem, isFavorite && styles.menuItemFavorite]}>
                        <View style={[styles.menuDot, isFavorite && styles.menuDotFavorite]} />
                        <View style={{ flex: 1 }}>
                          <Text style={[styles.menuText, isFavorite && styles.menuTextFavorite]}>
                            {foodName}
                            {isFavorite && ' ❤️'}
                          </Text>
                          {isFavorite && matchingKeywords.length > 0 && (
                            <Text style={styles.menuMatchingKeywordText}>
                              좋아하는 메뉴: {matchingKeywords.join(', ')}
                            </Text>
                          )}
                          {allergyInfo ? (
